"""
run_SP_DDPG_batch_test.py

Runs all 30 route files through the DDPG SIGNALIZED simulation in test mode.
Output: Olivarez_traci\batch_results\SP_DDPG_Results.xlsx

Metrics per run:
  - Mean Vehicle Time Loss (s)     — mean timeLoss from <tripinfo> tags
  - Mean Pedestrian Time Loss (s)  — mean timeLoss from <personinfo> tags
  - Mean Queue Length (m)          — avg jam across 11 detectors, every 60 s
  - Throughput (veh/hr)            — vehicles cleared, normalised to per-hour

Run alongside run_BP_DDPG_batch_test.py in a separate terminal.
"""
...
# ---------------------------------------------------------------------------
# Config — mirrors SignalizedPedestrianDDPG test mode exactly
# ---------------------------------------------------------------------------
SUMOCFG        = r'Olivarez_traci\SignalizedPed.sumocfg'
DEMAND_DIR     = r'Olivarez_traci\demand_test'
OUTPUT_DIR     = r'Olivarez_traci\batch_results\SP_DDPG'
XLSX_OUT       = r'Olivarez_traci\batch_results\SP_DDPG_Results.xlsx'
STEP_LENGTH    = 0.1
STATE_SIZE     = 19
ACTION_SIZE    = 3
...
METRIC_STEPS   = int(60 / STEP_LENGTH)          # every 60 s

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(os.path.dirname(XLSX_OUT), exist_ok=True)

# ---------------------------------------------------------------------------
# Load trained agent (TRAIN_MODE=0 → no noise)
# ---------------------------------------------------------------------------
agent = ddpg(
...
def subscribe_junctions():
    for jid in ["cluster_295373794_3477931123_7465167861","6401523012","3285696417"]:
        traci.junction.subscribeContext(jid, traci.constants.CMD_GET_PERSON_VARIABLE,
                                        10.0, [traci.constants.VAR_WAITING_TIME])

# ---------------------------------------------------------------------------
# State / phase helpers — exact match to original SP DDPG
# ---------------------------------------------------------------------------
...
def get_state(phase, state_mean, state_std):
    queues = []
    for det in ["e2_0","e2_1","e2_2","e2_3"]:
        queues.append(weighted_waits(det))
    for det in ["e2_4","e2_5","e2_6","e2_7","e2_8","e2_9","e2_10"]:
        queues.append(weighted_waits(det))

    for jid in ["cluster_295373794_3477931123_7465167861","6401523012","3285696417"]:
        sub = traci.junction.getContextSubscriptionResults(jid)
        queues.append(sum(d.get(traci.constants.VAR_WAITING_TIME, 0)
                          for d in sub.values()) * 1.25 if sub else 0)

    phase_enc   = to_categorical(phase // 2, num_classes=5).flatten()
    raw_state   = np.array(queues + list(phase_enc), dtype=np.float32)
    queue_array = np.array(queues, dtype=np.float32)

    # Running normalisation — same as original
    state_mean[:] = 0.99 * state_mean + 0.01 * raw_state
    state_std[:]  = 0.99 * state_std  + 0.01 * np.square(raw_state - state_mean)
    norm_state = (raw_state - state_mean) / (np.sqrt(state_std) + 1e-8)
    return norm_state.astype(np.float32), queue_array

def apply_phase(action_vals, current_phase):
    next_phase = (current_phase + 1) % 10
    for tl in ["cluster_295373794_3477931123_7465167861","6401523012","3285696417"]:
    ...
    if next_phase % 2 == 1:
        ...
        base = 20.0 if next_phase == 4 else 30.0
        durations = [float(max(5.0, min(60.0, base + float(np.clip(v,-1,1)*25))))
                     for v in action_vals]
        dur = durations[0]

    tls = ["cluster_295373794_3477931123_7465167861","6401523012","3285696417"]
    for tl, d in zip(tls, durations):
        traci.trafficlight.setPhaseDuration(tl, d)
    return next_phase, dur
...
def run_simulation(test):
    ...
    # Reset running normalisation stats per run
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if dur <= 0:
            norm_state, _ = get_state(phase, state_mean, state_std)

            if phase % 2 == 0:
                # Test mode: no noise
                action = agent.get_action(norm_state, add_noise=False)
                prev_act = action
            else:
                action = prev_act   # yellow — hold previous action

            phase, dur = apply_phase(action, phase)
...
def build_xlsx(results, title):
    ...
    ws["A2"] = "30 Runs  |  10 Normal  •  10 Slow  •  10 Traffic Jam  |  DDPG trained model (test mode)"
...
def main():
    ...
    wb = build_xlsx(results, "Signalized Pedestrian — DDPG Batch Test Results")
    wb.save(XLSX_OUT)
    print(f"\n{'='*65}\n  Saved: {XLSX_OUT}\n{'='*65}")
