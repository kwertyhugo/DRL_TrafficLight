"""
run_SP_DQN_batch_test.py

Runs all 30 route files through the DQN SIGNALIZED simulation in test mode.
Output: Olivarez_traci\batch_results\SP_DQN_Results.xlsx
...
Run alongside run_BP_DQN_batch_test.py in a separate terminal.
"""
...
# ---------------------------------------------------------------------------
# Config — mirrors SignalizedPedestrianDQN test mode exactly
# ---------------------------------------------------------------------------
SUMOCFG        = r'Olivarez_traci\SignalizedPed.sumocfg'
DEMAND_DIR     = r'Olivarez_traci\demand_test'
OUTPUT_DIR     = r'Olivarez_traci\batch_results\SP_DQN'
XLSX_OUT       = r'Olivarez_traci\batch_results\SP_DQN_Results.xlsx'
STEP_LENGTH    = 0.1
...
# ---------------------------------------------------------------------------
# Load trained agent (epsilon=0 → pure exploitation, no exploration)
# ---------------------------------------------------------------------------
agent = dqn(state_size=19, action_size=11, memory_size=2000, gamma=0.95,
            epsilon=0, epsilon_decay_rate=0.995, epsilon_min=0.01,
...
# ---------------------------------------------------------------------------
# State / phase helpers — exact match to original SP DQN
# ---------------------------------------------------------------------------
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if dur <= 0:
            if phase % 2 == 0:
                ...
                prev_act = agent.act(state)   # epsilon=0 → greedy
...
def main():
    ...
    wb = build_xlsx(results, "Signalized Pedestrian — DQN Batch Test Results")
    wb.save(XLSX_OUT)
    print(f"\n{'='*65}\n  Saved: {XLSX_OUT}\n{'='*65}")
