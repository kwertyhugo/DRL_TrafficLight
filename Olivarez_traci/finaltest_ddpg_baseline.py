"""
run_BP_DDPG_batch_test.py

Runs all 30 route files through the DDPG BASELINE simulation in test mode.
Output: Olivarez_traci\batch_results\BP_DDPG_Results.xlsx
...
Run alongside run_SP_DDPG_batch_test.py in a separate terminal.
"""
...
# ---------------------------------------------------------------------------
# Config — mirrors BaselinePedestrianDDPG test mode exactly
# ---------------------------------------------------------------------------
SUMOCFG        = r'Olivarez_traci\baselinePed.sumocfg'
DEMAND_DIR     = r'Olivarez_traci\demand_test'
OUTPUT_DIR     = r'Olivarez_traci\batch_results\BP_DDPG'
XLSX_OUT       = r'Olivarez_traci\batch_results\BP_DDPG_Results.xlsx'
STEP_LENGTH    = 0.05
STATE_SIZE     = 13
ACTION_SIZE    = 1
DETECTOR_IDS   = ["e2_4","e2_5","e2_6","e2_7","e2_8","e2_9","e2_10"]
DETECTOR_COUNT = len(DETECTOR_IDS)
METRIC_STEPS   = int(6 / STEP_LENGTH)           # every 6 s — matches original BP DDPG
...
# ---------------------------------------------------------------------------
# State / phase helpers — exact match to original BP DDPG
# ---------------------------------------------------------------------------
...
def apply_phase(action_val, current_phase):
    next_phase = (current_phase + 1) % 10
    traci.trafficlight.setPhase("cluster_295373794_3477931123_7465167861", next_phase)

    if next_phase in [2, 4]:
        base = 15.0
    elif next_phase % 2 == 0:
        base = 30.0
    else:
        base = 3.0
        action_val = 0.0   # yellow — no adjustment

    dur = float(max(5.0, min(60.0, base + float(np.clip(action_val,-1,1) * 20.0))))
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if dur <= 0:
            ...
            if phase % 2 == 0:
                ...
                action = np.zeros(ACTION_SIZE, dtype=np.float32)   # yellow — fixed

            phase, dur = apply_phase(action[0], phase)
...
def main():
    ...
    wb = build_xlsx(results, "Baseline Pedestrian — DDPG Batch Test Results")
    wb.save(XLSX_OUT)
    print(f"\n{'='*65}\n  Saved: {XLSX_OUT}\n{'='*65}")
