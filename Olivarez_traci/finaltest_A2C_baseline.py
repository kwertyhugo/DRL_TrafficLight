"""
run_BP_A2C_batch_test.py

Runs all 30 route files through the A2C BASELINE simulation in test mode.
Output: Olivarez_traci\batch_results\BP_A2C_Results.xlsx

Metrics per run:
  - Mean Vehicle Time Loss (s)     — mean timeLoss from <tripinfo> tags
  - Mean Pedestrian Time Loss (s)  — mean timeLoss from <personinfo> tags
  - Mean Queue Length (m)          — avg jam length across 7 detectors, every 60 s
  - Throughput (veh/hr)            — vehicles cleared, normalised to per-hour

Run alongside run_SP_A2C_batch_test.py in a separate terminal.
"""
...
OUTPUT_DIR     = r'Olivarez_traci\batch_results\BP_A2C'
XLSX_OUT       = r'Olivarez_traci\batch_results\BP_A2C_Results.xlsx'
MODEL_PATH     = r'Olivarez_traci\models_A2C_Baseline\A2C_Baseline_Agent.keras'
STEP_LENGTH    = 0.05
DETECTOR_IDS   = ["e2_4","e2_5","e2_6","e2_7","e2_8","e2_9","e2_10"]
DETECTOR_COUNT = len(DETECTOR_IDS)
METRIC_STEPS   = int(60 / STEP_LENGTH)
ACTION_SPACE   = (-15, -10, -5, 0, 5, 10, 15)
...
# Load model
# ---------------------------------------------------------------------------
agent = a2c(state_size=13, action_size=7, gamma=0.99, learning_rate=0.0005,
...
def apply_phase(action_idx, current_phase):
    ...
    if next_phase == 2 or next_phase == 4:
        base = 15
    elif next_phase % 2 == 0:
        base = 30
    else:
...
def run_simulation(test):
    ...
    phase    = 0
    dur      = 30.0
    prev_act = 3   # index of 0 in ACTION_SPACE
...
def build_xlsx(results):
    ...
    ws["A1"] = "Baseline Pedestrian — A2C Batch Test Results"
