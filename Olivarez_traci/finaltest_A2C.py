...
OUTPUT_DIR     = r'Olivarez_traci\batch_results\SP_A2C'
XLSX_OUT       = r'Olivarez_traci\batch_results\SP_A2C_Results.xlsx'
MODEL_PATH     = r'Olivarez_traci\models_A2C\A2C_Unified_Agent.keras'
STEP_LENGTH    = 0.1
...
# Load model
# ---------------------------------------------------------------------------
agent = a2c(state_size=19, action_size=11, gamma=0.99, learning_rate=0.0005,
            entropy_coef=0.08, value_coef=0.5, max_grad_norm=1.0,
...
def weighted_waits(det_id):
    ...
    return sum(d.get(traci.constants.VAR_WAITING_TIME,0) * wmap.get(d.get(traci.constants.VAR_TYPE,"car"),1.0)
               for d in data.values())

def get_state(phase):
    queues = [weighted_waits(d) for d in
              ["e2_0","e2_1","e2_2","e2_3","e2_4","e2_5","e2_6","e2_7","e2_8","e2_9","e2_10"]]
    for jid in ["cluster_295373794_3477931123_7465167861","6401523012","3285696417"]:
...
def apply_phase(action_idx, current_phase):
    ...
    if next_phase % 2 == 1:
        ...
        base = 20 if next_phase == 2 else 30
...
def parse_tripinfo(trips_xml):
    try:
        root = ET.parse(trips_xml).getroot()
    except (FileNotFoundError, ET.ParseError):
        return 0.0, 0.0
    veh_losses, ped_losses = [], []
    for tag in root:
        tl = tag.get("timeLoss")
        if tl is None: continue
        if tag.tag == "tripinfo":
            veh_losses.append(float(tl))
        elif tag.tag == "personinfo":
            ped_losses.append(float(tl))
    return (sum(veh_losses)/len(veh_losses) if veh_losses else 0.0,
            sum(ped_losses)/len(ped_losses) if ped_losses else 0.0)
...
def run_simulation(test):
    ...
    phase    = 0
    dur      = 30.0
    prev_act = 5   # index of 0 in ACTION_SPACE
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if dur <= 0:
            if phase % 2 == 0:
                ...
                prev_act = agent.act(state, training=False)
...
def build_xlsx(results):
    ...
    ws["A1"] = "Signalized Pedestrian — A2C Batch Test Results"
    ws["A1"].font = Font(bold=True, name="Arial", size=13); ws["A1"].alignment = _c()
    ws.row_dimensions[1].height = 24

    ws.merge_cells(f"A2:{last}2")
    ws["A2"] = "30 Runs  |  10 Normal  •  10 Slow  •  10 Traffic Jam  |  A2C trained model (test mode)"
