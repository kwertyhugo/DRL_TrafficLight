"""
run_Balibago_BP_DQN_batch_test.py

Runs all 30 Balibago demand_test route files through the DQN BASELINE
simulation in test mode and writes:
    Balibago_traci/batch_results/BP_DQN_Results.xlsx

Matches BaselinePedestrianDQN.py exactly:
  - baselinePed.sumocfg,  stepLength=0.1,  MAX_STEPS=567000
  - North (4902876117): 6-phase cycle (%6), state=12 (8q + 4ph), norm /1000
    base = {0:45, 2:130, 4:30, 6:90}, action clamp max 180
  - South (12188714):   6-phase cycle (%6), state=9  (5q + 4ph), norm /1000
    base = {0:30, 2:30,  4:45},        action clamp max 180
  - actionSpace = (-25..+25), yellow=5
...
# North: 6-phase cycle, base durations
NORTH_TL     = "4902876117"
NORTH_PHASES = 6
NORTH_BASE   = {0: 45, 2: 130, 4: 30, 6: 90}

# South: 6-phase cycle, base durations
SOUTH_TL     = "12188714"
SOUTH_PHASES = 6
...
# ---------------------------------------------------------------------------
# Load agents  (epsilon=0 → greedy, no exploration)
# ---------------------------------------------------------------------------
...
NorthAgent.load()
SouthAgent.load()
print(f"\n{'='*65}\n  North DQN + South DQN models loaded.\n{'='*65}")
...
# ---------------------------------------------------------------------------
# State helpers — exact match to BP DQN original
# ---------------------------------------------------------------------------
...
def get_north_state(phase):
    q  = np.array([weighted_waits(f"e2_{i}") for i in range(8)]) / 1000.0
    ph = to_categorical(phase // 2, num_classes=4).flatten()
    return np.concatenate([q, ph]).astype(np.float32)          # 12

def get_south_state(phase):
    q  = np.array([weighted_waits(f"e2_{i}") for i in range(8, 13)]) / 1000.0
    ph = to_categorical(phase // 2, num_classes=4).flatten()
    return np.concatenate([q, ph]).astype(np.float32)          # 9
...
def run_simulation(test):
    ...
    n_phase, n_dur = 0, float(NORTH_BASE.get(0, 30))
    s_phase, s_dur = 0, float(SOUTH_BASE.get(0, 30))
...
def main():
    ...
    wb = build_xlsx(results,
                    "Balibago Baseline Pedestrian — DQN Batch Test Results",
