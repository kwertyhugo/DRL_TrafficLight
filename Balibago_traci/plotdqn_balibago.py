...
SMOOTHING_WINDOW = 20
# Approximately how many rows to skip (can adjust based on CSV size)
# For smaller files, we'll skip fewer rows
SKIP_ROWS_THRESHOLD = 1000
...
def plot_agent_history(agent_name, csv_path):
    ...
    # Dynamically set skip rows based on file size (skip ~25% or minimum 100)
    skip_rows = max(100, total_rows // 4) if total_rows > SKIP_ROWS_THRESHOLD else 0
    
    if skip_rows > 0:
        df = df_full.iloc[skip_rows:].reset_index(drop=True)
        print(f"  Total rows: {total_rows:,}. Skipped first {skip_rows:,} rows. Plotting {len(df):,} remaining rows.")
    else:
...
def main():
    print("=" * 70)
    print("Generating DQN training plots for Balibago (Signalized)...")
