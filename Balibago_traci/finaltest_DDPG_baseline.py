"""
run_Balibago_BP_DDPG_batch_test.py

Runs all 30 Balibago demand_test route files through the DDPG BASELINE
simulation in test mode and writes:
    Balibago_traci/batch_results/BP_DDPG_Results.xlsx

Matches BaselinePedestrianDDPG.py exactly:
  - baselinePed.sumocfg,  stepLength=0.1
  - North (4902876117): 8-phase cycle (%8), state=12 (8q + 4ph), norm /100,
    base={0:45, 2:130, 4:30, 6:90}, ACTION_SCALE=25, yellow=5
  - South (12188714):   6-phase cycle (%6), state=8  (5q + 3ph), norm /100,
    base={0:30, 2:30,  4:45},        ACTION_SCALE=25, yellow=5
  - TRAIN_MODE=0 → get_action(add_noise=False)
  - Metric interval: every 60 s (600 steps)
  - Models: Baseline_North2_DDPGAgent / Baseline_South2_DDPGAgent
...
OUTPUT_DIR     = 'Balibago_traci/batch_results/BP_DDPG'
XLSX_OUT       = 'Balibago_traci/batch_results/BP_DDPG_Results.xlsx'
STEP_LENGTH    = 0.1
...
NORTH_TL     = "4902876117"
NORTH_PHASES = 8
NORTH_BASE   = {0: 45.0, 2: 130.0, 4: 30.0, 6: 90.0}

SOUTH_TL     = "12188714"
SOUTH_PHASES = 6
SOUTH_BASE   = {0: 30.0, 2: 30.0, 4: 45.0}

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(os.path.dirname(XLSX_OUT), exist_ok=True)

# ---------------------------------------------------------------------------
# Load agents (TRAIN_MODE=0 → no noise)
# ---------------------------------------------------------------------------
NorthAgent = ddpg(
    state_size=12, action_size=1,
    action_low=np.array([-1.0], dtype=np.float32),
    action_high=np.array([1.0],  dtype=np.float32),
    actor_lr=0.0001, critic_lr=0.001,
    name='Baseline_North2_DDPGAgent'
)
SouthAgent = ddpg(
    state_size=8, action_size=1,
    action_low=np.array([-1.0], dtype=np.float32),
    action_high=np.array([1.0],  dtype=np.float32),
    actor_lr=0.0001, critic_lr=0.001,
    name='Baseline_South2_DDPGAgent'
)
...
if os.path.exists(model_path) and os.listdir(model_path):
    try:
        NorthAgent.load()
        SouthAgent.load()
        print(f"\n{'='*65}\n  North DDPG + South DDPG models loaded.\n{'='*65}")
...
for scenario in ["normal", "slow", "jam"]:
    for i in range(1, 11):
        TEST_PLAN.append({
            ...
            "trips_out": os.path.join(OUTPUT_DIR, f"BP_DDPG_trips_{scenario}_{i:02d}.xml"),
            "stats_out": os.path.join(OUTPUT_DIR, f"BP_DDPG_stats_{scenario}_{i:02d}.xml"),
        })
...
# ---------------------------------------------------------------------------
# State helpers — exact match to BP DDPG original
# ---------------------------------------------------------------------------
...
def get_north_state(phase):
    # 8 detectors + 4 phase one-hot = 12; norm /100
...
def run_simulation(test):
    ...
    n_phase, n_dur = 0, NORTH_BASE[0]
    s_phase, s_dur = 0, SOUTH_BASE[0]
    n_prev_act = np.zeros(1, dtype=np.float32)
    s_prev_act = np.zeros(1, dtype=np.float32)
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if south_decision:
            ...
            s_act = np.zeros(1, dtype=np.float32)   # yellow — fixed, no adjustment
        else:
            s_act = s_prev_act

        if n_dur <= 0:
            n_phase, n_dur = apply_north_phase(n_act[0], n_phase)
        if s_dur <= 0:
            s_phase, s_dur = apply_south_phase(s_act[0], s_phase)
...
def main():
    ...
    wb = build_xlsx(results,
                    "Balibago Baseline Pedestrian — DDPG Batch Test Results",
