"""
run_Balibago_BP_NoDRL_batch_test.py

Runs all 30 Balibago demand_test route files through the NoDRL BASELINE
simulation and writes:
    Balibago_traci/batch_results/BP_NoDRL_Results.xlsx

Matches BaselinePedestrianNODRL.py exactly:
  - baselinePed.sumocfg
  - stepLength = 0.1,  max_sim_steps = 576000
  - North (4902876117): 8-phase, green={0:45, 2:130, 4:30, 6:90}, yellow=5
  - South (12188714):   6-phase, green={0:30, 2:30,  4:45},        yellow=5
...
OUTPUT_DIR     = 'Balibago_traci/batch_results/BP_NoDRL'
XLSX_OUT       = 'Balibago_traci/batch_results/BP_NoDRL_Results.xlsx'
STEP_LENGTH    = 0.1
MAX_STEPS      = 576000
...
for scenario in ["normal", "slow", "jam"]:
    for i in range(1, 11):
        TEST_PLAN.append({
            ...
            "trips_out": os.path.join(OUTPUT_DIR, f"BP_NoDRL_trips_{scenario}_{i:02d}.xml"),
            "stats_out": os.path.join(OUTPUT_DIR, f"BP_NoDRL_stats_{scenario}_{i:02d}.xml"),
        })
...
def run_simulation(test):
    ...
    n_phase, n_dur = 0, float(NORTH_GREEN[0])
    s_phase, s_dur = 0, float(SOUTH_GREEN[0])
    step = 0
...
def main():
    ...
    wb = build_xlsx(results,
                    "Balibago Baseline Pedestrian — NoDRL Batch Test Results",
