...
# ==========================================
# North: 8 Detectors + 4 Phase One-Hot = 12 State Size (no pedestrian)
NorthAgent = ddpg(
    state_size=12,
    action_size=1,
...
# South: 5 Detectors + 3 Phase One-Hot = 8 State Size (no pedestrian, 6-phase cycle)
SouthAgent = ddpg(
    state_size=8,
    action_size=1,
    action_low=action_low,
...
Sumo_config = [
    ...
    '--statistic-output', r'Balibago_traci/output_DDPG/test_jam_baseline_stats.xml',
    '--tripinfo-output', r'Balibago_traci/output_DDPG//test_jam_baseline_trips.xml'
]
...
southCurrentPhase = 0
southCurrentPhaseDuration = 30.0   # Starting duration = base of phase 0

# Fixed base durations (used as reference; agent adjusts green phases)
# North: 8-phase cycle (0,2,4,6 are green; 1,3,5,7 are yellow)
NORTH_BASE_DURATIONS = {0: 45.0, 2: 130.0, 4: 30.0, 6: 90.0}
# South: 6-phase cycle (0,2,4 are green; 1,3,5 are yellow)
SOUTH_BASE_DURATIONS = {0: 30.0, 2: 30.0, 4: 45.0}
YELLOW_DURATION = 5.0
...
def _northIntersection_queue():
    """Returns 8 weighted detector queues (no pedestrian)."""
    queues = [_weighted_waits(f"e2_{i}") for i in range(8)]
    return queues  # 8 values total

def _southIntersection_queue():
    """Returns 5 weighted detector queues (no pedestrian)."""
    queues = [_weighted_waits(f"e2_{i}") for i in range(8, 13)]
    return queues  # 5 values total
...
# NOTE: No pedestrian junction subscriptions needed for baseline
...
try:
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if north_decision_needed:
            raw_q = _northIntersection_queue()          # 8 values (8 detectors, no ped)
            queue_arr = np.array(raw_q, dtype=np.float32)
            n_norm_queue = queue_arr / 100.0

            # 4 green phases in 8-phase cycle → num_classes=4
        ...
        if south_decision_needed:
            raw_q = _southIntersection_queue()          # 5 values (5 detectors, no ped)
            queue_arr = np.array(raw_q, dtype=np.float32)
            s_norm_queue = queue_arr / 100.0

            # 3 green phases in 6-phase cycle → num_classes=3
            s_phase_oh = to_categorical(southCurrentPhase // 2, num_classes=3).flatten()
            obs_south = np.concatenate([s_norm_queue, s_phase_oh]).astype(np.float32)  # 5 + 3 = 8
    ...
    if TRAIN_MODE == 1:
        ...
        save_history(
            './Balibago_traci/output_DDPG/baseline2_North_history.csv',
            headers, reward_history_N, actor_loss_N, critic_loss_N, TRAIN_FREQUENCY
        )
        save_history(
            './Balibago_traci/output_DDPG/baseline2_South_history.csv',
