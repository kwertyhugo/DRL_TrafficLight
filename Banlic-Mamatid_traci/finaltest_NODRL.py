"""
run_BanlicMamatid_NoDRL_batch_test.py

Runs all 30 route files through the NoDRL SIGNALIZED simulation.
Output: Banlic-Mamatid_traci/batch_results/NoDRL_Results.xlsx

Matches SignalizedPedestrianNODRL.py exactly:
  - signalizedPed.sumocfg,  stepLength=0.1,  max_sim_steps=576000
  - Two synced junctions: 253768576 + 253499548
  - 10-phase cycle %10, green={0:30,2:30,4:45,6:60,8:25}, yellow=5
  - 6 detectors: e2_0 – e2_5,  metric every 60 s
...
OUTPUT_DIR     = 'Banlic-Mamatid_traci/batch_results/NoDRL'
XLSX_OUT       = 'Banlic-Mamatid_traci/batch_results/NoDRL_Results.xlsx'
STEP_LENGTH    = 0.1
MAX_STEPS      = 576000
...
for scenario in ["normal", "slow", "jam"]:
    for i in range(1, 11):
        TEST_PLAN.append({
            ...
            "trips_out": os.path.join(OUTPUT_DIR, f"NoDRL_trips_{scenario}_{i:02d}.xml"),
            "stats_out": os.path.join(OUTPUT_DIR, f"NoDRL_stats_{scenario}_{i:02d}.xml"),
        })
...
def apply_fixed_timing(phase, dur):
    dur -= STEP_LENGTH
    if dur <= 0:
        phase = (phase + 1) % TOTAL_PHASES
        traci.trafficlight.setPhase(JUNCTION_1, phase)
        traci.trafficlight.setPhase(JUNCTION_2, phase)
        dur = YELLOW_DURATION if phase % 2 == 1 else GREEN_DURATIONS.get(phase, 30)
...
def run_simulation(test):
    ...
    phase, dur = 0, float(GREEN_DURATIONS[0])
    step = 0
    jam_total = 0.0
    tp_total  = 0
    obs       = 0
    ...
    while traci.simulation.getMinExpectedNumber() > 0 and step < MAX_STEPS:
        phase, dur = apply_fixed_timing(phase, dur)
...
# ---------------------------------------------------------------------------
# Excel builder (shared by all 3 scripts)
# ---------------------------------------------------------------------------
...
def main():
    ...
    wb = build_xlsx(results,
                    "Banlic-Mamatid Signalized — NoDRL Batch Test Results",
